...
static inline int
rx_last_ack(struct tle_tcp_stream *s, uint32_t ack)
{
	if (ack != s->tcb.snd.nxt)
		return -EINVAL;
...
static inline int
rx_fin(struct tle_tcp_stream *s, uint32_t state, uint32_t seq,
	struct resp_info *rsp)
{
	if (s->tcb.rcv.nxt == seq && state == TCP_ST_ESTABLISHED) {
	...
	uint32_t num)
{
	...
	for (i = 0; i != num; i++) {
		...
		ret = rx_check_seq(&s->tcb, mb[i], si[i].seq, len);
		if (ret != 0) {
			...
			ret = rx_check_ack(&s->tcb, si[i].ack);
		if (ret == 0) {

			if (state == TCP_ST_LAST_ACK) {
				ret = rx_last_ack(s, si[i].ack);
				if (ret == 0)
					break;
			}
			...
			if ((pi->tf.flags & TCP_FLAG_FIN) != 0)
				ret = rx_fin(s, state, si[i].seq, &rsp);

			/* valid packet, payload no processed*/
