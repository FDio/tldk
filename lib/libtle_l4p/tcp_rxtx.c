	...
	const union pkt_info *pi)
{
	...
	type = pi->tf.type;
	if (type >= TLE_VNUM || pi->csf != 0 || dev->dp[type] == NULL)
		return NULL;
...
rx_listen(struct tle_tcp_stream *s, struct stbl *st, const union pkt_info *pi,
	const union seg_info *si, uint32_t ts, struct rte_mbuf *mb)
{
	int32_t rc;
...
static inline uint32_t
rx_stream(struct tle_tcp_stream *s, struct stbl *st, uint32_t ts,
	...
	uint32_t num)
{
	...
	if (state == TCP_ST_LISTEN) {
		...
		for (i = 0; i != num && ret != 0; i++) {
			ret = rx_listen(s, st, pi, &si[i], ts, mb[i]);
			if (ret != 0) {
				rc[k] = -ret;
				rp[k] = mb[i];
				k++;
	...
	struct rte_mbuf *rp[], int32_t rc[], uint16_t num)
{
	...
	/*if ((ol_flags & (PKT_RX_IP_CKSUM_BAD | PKT_RX_L4_CKSUM_BAD)) != 0) {
		for (i = 0; i != num; i++) {
			if (check_pkt_csum(pkt[i], pi[i].tf.type,
					IPPROTO_TCP) != 0)
				pi[i].tf.type = TLE_VNUM;
		}
	}*/
	...
	for (i = 0; i != num; i += j) {
		...
		if (s != NULL) {
			...
			n = rx_stream(s, st, ts, pi + i, si + i, pkt + i,
				rp + k, rc + k, j);
	...
	const struct tle_tcp_accept_param *prm)
{
	...
	get_pkt_info(prm->syn.pkt, &pi, &si);
