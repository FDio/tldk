...
start_fast_retransmit(struct tle_tcp_stream *s)
{
	...
	TCP_LOG(INFO, "%s(%p): state=%u, "
		"snd.una=%lu, flight_size=%lu, "
		"snd.rcvr=%lu, snd.fastack=%u, "
		"wnd=%u, cwnd=%u, ssthresh=%u, "
		"bytes sent=%lu, pkt remain=%u;\n",
		__func__, s, s->tcb.state,
		s->tcb.snd.una, s->tcb.snd.rcvr - s->tcb.snd.una,
		s->tcb.snd.rcvr, s->tcb.snd.fastack,
		s->tcb.snd.wnd, s->tcb.snd.cwnd, s->tcb.snd.ssthresh,
		s->tcb.snd.rcvr - s->tcb.snd.iss, tcp_txq_nxt_cnt(s));
}
...
stop_fast_retransmit(struct tle_tcp_stream *s)
{
	...
	TCP_LOG(INFO, "%s(%p): state=%u, "
		"snd.una=%lu, snd.nxt=%lu, flight_size=%lu, "
		"snd.rcvr=%lu, snd.fastack=%u, "
		"wnd=%u, cwnd=%u, ssthresh=%u, "
		"bytes sent=%lu, pkt remain=%u;\n",
		__func__, s, s->tcb.state,
		s->tcb.snd.una, s->tcb.snd.nxt, s->tcb.snd.nxt - s->tcb.snd.una,
		s->tcb.snd.rcvr, s->tcb.snd.fastack,
		s->tcb.snd.wnd, s->tcb.snd.cwnd, s->tcb.snd.ssthresh,
	...
	const struct dack_info *tack)
{
	...
	n = tack->ack - (uint32_t)s->tcb.snd.una;
