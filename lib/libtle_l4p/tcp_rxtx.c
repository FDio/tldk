...
static inline int
rx_check_seqack(struct tcb *tcb, const struct rte_mbuf *mb,
	uint32_t seq, uint32_t ack, uint32_t len)
{
	int32_t rc;

	rc = rx_check_seq(tcb, mb, seq, len); 
	...
	struct resp_info *rsp)
{
	...
	if (rx_check_seq(&s->tcb, mb, si->seq, len) != 0) {
	...
	struct rte_mbuf *mb[], uint32_t num) 
{
	...
	if (seq == s->tcb.rcv.nxt) {
		...
		t = tcp_rx_ino_enqueue(s, seq, len, mb, num);
		if (t != num)
			TCP_LOG(ERR,
	...
	struct resp_info *rsp)
{
	...
	ret = rx_check_seqack(&s->tcb, mb, seq, si->ack, plen);
	if (ret != 0)
		return ret;
...
static inline uint32_t
rx_data_ack(struct tle_tcp_stream *s, const union seg_info si[],
	...
	uint32_t num)
{
	...
	for (i = 0; i != num; i = j) {
		...
		ret = rx_check_seqack(&s->tcb, mb[i], seq, si[i].ack, plen);
		...
		for (tlen = plen; j != num; tlen += plen, j++) {
			...
			/* wrong SEQ/ACK */
			ret = rx_check_seqack(&s->tcb, mb[j], si[j].seq,
				si[j].ack, plen);
			if (ret != 0) {
				rp[k] = mb[j];
				rc[k] = -ret;
...
static inline uint32_t
rx_stream(struct tle_tcp_stream *s,
	const union pkt_info *pi, const union seg_info si[],
	struct rte_mbuf *mb[], struct rte_mbuf *rp[], int32_t rc[],
	uint32_t num)
{
	...
	/* normal data/ack packets */
		...
		n = rx_data_ack(s, si, mb, rp, rc, num);
		rsp.flags |= TCP_FLAG_ACK;
		k += num - n;
		i = num;
	...
	uint32_t num)
{
	...
	if (state == TCP_ST_LISTEN) {
		...
		else if(s->rx.cb.func != NULL && rte_ring_count(s->rx.lq) == 1)
			s->rx.cb.func(s->rx.cb.data, &s->s);

	} else {
		i = rx_stream(s, pi, si, mb, rp, rc, num);
		k = num - i;
	}
	...
	const union pkt_info *pi, const union seg_info *si)
{
	...
	cs->tcb.rcv.wnd = cs->rx.lq->prod.mask << cs->tcb.rcv.wscale;
	...
	uint32_t num)
{
	...
	for (i = 0; i != n; i++) {
		...
		/* prepare new stream */
		cs = TCP_STREAM(rs[i]);
		rc = accept_fill_stream(s, cs, prm + i, pi + i, si + i);
		if (rc != 0)
			break;
	}
...
tle_tcp_stream_recv(struct tle_stream *ts, struct rte_mbuf *pkt[], uint16_t num)
{
	...
	s = TCP_STREAM(ts);
	n = rte_ring_mc_dequeue_burst(s->rx.lq, (void **)pkt, num);
	if (n == 0)
		return 0;
