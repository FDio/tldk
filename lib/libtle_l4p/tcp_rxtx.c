	...
	uint32_t seq, uint8_t hlen, uint8_t flags)
{
	...
	l4h->data_off = hlen / TCP_DATA_ALIGN;
	l4h->tcp_flags = flags;
...
tcp_fill_mbuf(struct rte_mbuf *m, const struct tle_tcp_stream *s,
	const struct tle_dest *dst, uint64_t ol_flags, uint32_t seq,
	uint32_t flags, uint32_t pid)
{
	...
	fill_tcph(l4h, &s->tcb, s->s.port, seq, l4, flags);
	...
	struct rte_mbuf *const m[], uint32_t num)
{
	...
	for (i = 0; i != num; i++) {
		...
		/* prepare for TX */
		if (tcp_fill_mbuf(m[i], s, dst, ol_flags, seq, flags,
				pid + i) != 0)
			break;
	...
	struct rte_mbuf *m)
{
	...
	rc = tcp_fill_mbuf(m, s, &dst, 0, seq, TCP_FLAG_SYN | TCP_FLAG_ACK,
		pid);
	if (rc == 0)
		rc = send_pkt(s, dev, m);
