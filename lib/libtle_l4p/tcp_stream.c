...
tle_tcp_stream_close(struct tle_stream *ts)
{
	...
	if (s->lrs == NULL)
		...
		stbl_del_stream(CTX_TCP_STLB(ctx), s->ste, s);
		s->ste = NULL;
	}
	...
	struct tle_tcp_stream_param *prm)
{
	...
	if (s->s.type == TLE_V4) {
		...
		rin4 = (struct sockaddr_in *)&prm->remote_addr;

		prm->local_addr.ss_family = AF_INET;
		prm->remote_addr.ss_family = AF_INET;
		...
		rin6 = (struct sockaddr_in6 *)&prm->remote_addr;

		prm->local_addr.ss_family = AF_INET6;
		prm->remote_addr.ss_family = AF_INET6;
