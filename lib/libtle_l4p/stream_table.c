...
stbl_init(struct stbl *st, uint32_t num, int32_t socket)
{
	...
	hprm.name = buf;
	hprm.entries = RTE_MAX(5 * num / 4, 0x10U);
	...
	st->ht[TLE_V4] = rte_hash_create(&hprm);
	if (st->ht[TLE_V4] == NULL)
		rc = (rte_errno != 0) ? -rte_errno : -ENOMEM;

	if (rc == 0) {
		snprintf(buf, sizeof(buf), "stbl6@%p", st);
		hprm.key_len = sizeof(struct stbl6_key);
		st->ht[TLE_V6] = rte_hash_create(&hprm);
		if (st->ht[TLE_V6] == NULL)
			rc = (rte_errno != 0) ? -rte_errno : -ENOMEM;
	}

	if (rc == 0) {
		sz = sizeof(*st->ent) * num;
		st->ent = rte_zmalloc_socket(NULL, sz, RTE_CACHE_LINE_SIZE,
			socket);
		if (st->ent == NULL)
			rc = -ENOMEM;
		else
