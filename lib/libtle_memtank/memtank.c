...
init_chunk(struct memtank *mt, struct memchunk *ch)
{
	...
	obj = (struct memobj *)(ch + memchunk_meta_size(n));
...
grow_chunk(struct memtank *mt, uint32_t num)
{
	...
	if (n != 0) {
		...
		TAILQ_CONCAT(&ls, &fls->chunk, link);
		fls->chunk = ls;
	...
	struct mchunk_head *uls, void *obj[], uint32_t nb_obj)
{
	...
	for (ch = TAILQ_FIRST(&ls->chunk);
			...
			ch = TAILQ_NEXT(ch, link), n += k) {

		k = RTE_MAX(nb_obj - n, ch->nb_free);
		l = ch->nb_free - k;
...
tle_memtank_grow(struct tle_memtank *t)
{
	...
	num = RTE_ALIGN_MUL_CEIL(k, mt->prm.nb_obj_chunk);
...
tle_memtank_shrink(struct tle_memtank *t)
{
	...
	num = RTE_ALIGN_MUL_CEIL(t->min_free, mt->prm.nb_obj_chunk);
