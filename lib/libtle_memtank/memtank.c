...
obj_dbg_alloc(struct memtank *mt, void * const obj[], uint32_t nb_obj)
{
	...
	for (i = 0; i != nb_obj; i++) {
		...
		RTE_VERIFY(po->red_zone1 == RED_ZONE_V1);
		RTE_VERIFY(po->red_zone2 == RED_ZONE_V2);
		RTE_VERIFY(po->dbg.nb_alloc == po->dbg.nb_free);
		po->dbg.nb_alloc++;
	}
}
...
obj_dbg_free(struct memtank *mt, void * const obj[], uint32_t nb_obj)
{
	...
	for (i = 0; i != nb_obj; i++) {
		...
		RTE_VERIFY(po->red_zone1 == RED_ZONE_V1);
		RTE_VERIFY(po->red_zone2 == RED_ZONE_V2);
		RTE_VERIFY(po->dbg.nb_alloc == po->dbg.nb_free + 1);
		po->dbg.nb_free++;
	}
}
