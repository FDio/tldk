...
struct tle_dev
*tcp_stream_base::setup_dev(struct tle_ctx *ctx,
		struct tle_dev_param *dev_prm) {
...
void
tcp_stream_base::setup_dev_prm(struct tle_dev_param *dev_prm,
		char const *ipv4, char const *ipv6) {
...
protected:
	virtual void SetUp(void)
	{
		...
		ret = setup_stream_prm(&stream_prm, ipv4_laddr, ipv4_raddr, l_port, r_port);
		ASSERT_EQ(ret, 0);
		setup_stream_prm(&stream_prm6, ipv6_laddr, ipv6_raddr, l_port, r_port);
		ASSERT_EQ(ret, 0);
