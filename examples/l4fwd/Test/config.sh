...
# name of local interface to be used by DPDK
LOCAL_IFACE=enp138s0f0
...
# set rbufs to open for l4fwd
L4FWD_RBUFS=0x100
# set sbufs to open for l4fwd
L4FWD_SBUFS=0x100
# set streams to open for l4fwd
L4FWD_STREAMS=0x1000
...
L4FWD_CMD_PRM="--listen --tcp --enable-arp --promisc --rbufs ${L4FWD_RBUFS} \
--sbufs ${L4FWD_SBUFS} --streams ${L4FWD_STREAMS}"
...
if [[ ${ipv4} -eq 1 ]]
then
	L4FWD_PORT_PRM="port=${DPDK_PORT},lcore=${L4FWD_BECORE},rx_offload=0xf\
,tx_offload=0xf,ipv4=${addr}"
elif [[ ${ipv6} -eq 1 ]]
then
	L4FWD_PORT_PRM="port=${DPDK_PORT},lcore=${L4FWD_BECORE},rx_offload=0xf\
,tx_offload=0xf,ipv6=${addr}"
fi

# other variables--------------------------------------------------------------

# check if directories on remote are set, if not make one
if ssh ${REMOTE_HOST} [[ ! -d ${REMOTE_DIR} ]]
then
	ssh ${REMOTE_HOST} mkdir ${REMOTE_DIR}
fi
if ssh ${REMOTE_HOST} [[ ! -d ${REMOTE_OUTDIR} ]]
then
	ssh ${REMOTE_HOST} mkdir ${REMOTE_OUTDIR}
fi
if ssh ${REMOTE_HOST} [[ ! -d ${REMOTE_RESDIR} ]]
then
	ssh ${REMOTE_HOST} mkdir ${REMOTE_RESDIR}
fi
...
update_results()
{
	...
	$(ssh ${REMOTE_HOST} "awk '/real/{print \$2}' ${REMOTE_RESDIR}/${file}.result.${it} >> ${REMOTE_RESDIR}/results.out")

	# add file and status of test to results
	if [[ ${status} -ne 0 ]]
	then
		$(ssh ${REMOTE_HOST} "sed -i '$ s_.*_[FAIL]\t&_' ${REMOTE_RESDIR}/results.out")
	else
		$(ssh ${REMOTE_HOST} "sed -i '$ s_.*_[OK]\t&_' ${REMOTE_RESDIR}/results.out")
	fi

	$(ssh ${REMOTE_HOST} "sed -i '$ s_.*_${file}\t&_' ${REMOTE_RESDIR}/results.out")
...
configure_ip4_remote()
{
	...
	ssh ${REMOTE_HOST} ifconfig ${REMOTE_IFACE} \
		${REMOTE_IPV4}/${MASK_IPV4} up
	if_verbose ssh ${REMOTE_HOST} ifconfig ${REMOTE_IFACE}
...
lcore=${L4FWD_FECORE},belcore=${L4FWD_BECORE},op=echo,laddr=${addr}\
,lport=${port},raddr=${REMOTE_IPV4},rport=0
EOF
...
config6_fe()
{
	cat <<EOF > ${L4FWD_FE_CFG_FILE}
lcore=${L4FWD_FECORE},belcore=${L4FWD_BECORE},op=echo,laddr=${addr}\
,lport=${port},raddr=${REMOTE_IPV6},rport=0
EOF
