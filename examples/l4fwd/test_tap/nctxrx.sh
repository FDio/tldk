...
# packets are out of order/lost. Script is operating on local linux machine.
# l4fwd application is being run by the script, which sets up the tap interface.
# Created interface is serving a connection for l4fwd and netcat within
# the same OS.
...
# Netcat(TAP) --(TCP out of order)--> (TAP)L4FWD(TAP) --
#	--(TCP with correct order)--> (TAP)Netcat(validation)
...
exec 4>&-"
	...
	if_verbose netstat -na | grep ${TCP_PORT}
	...
	if_verbose ps -ef | grep "nc " | grep -v grep
...
then
	if_verbose echo -e "Sending test file"
	run_test ${file}
	...
	do
		...
		x=$(mktemp $(basename $0).${size}MB.XXX \
			-p ${REMOTE_DIR})

		dd if=/dev/urandom of=${x} bs=1M \
