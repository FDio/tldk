	...
	struct sockaddr_storage *ra)
{
	...
	if (fes->proto == TLE_PROTO_TCP)
		sprintf(output + n, "tcp_stats={con_acc=%lu, con_rej=%lu};};\n",
			fes->tcp_stat.acc, fes->tcp_stat.rej);
	else
		sprintf(output + n, "};\n");

	RTE_LOG(INFO, USER1, "%s", output);
}
