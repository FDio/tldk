...
qidx_from_hash_index(uint32_t hash, uint32_t align_nb_q)
{
	...
	nb_bit = (sizeof(uint32_t)*8) - __builtin_clz(align_nb_q-1);
	q = (hash & 1);
	for (i = 1; i < nb_bit; i++) {
		hash >>= 1;
		q <<= 1;
...
static int
update_rss_conf(struct netbe_port *uprt, struct rte_eth_dev_info *dev_info,
...
static int
update_rss_reta(struct netbe_port *uprt, struct rte_eth_dev_info *dev_info)
{
...
log_netbe_prt(const struct netbe_port *uprt)
{
	...
	RTE_LOG(NOTICE, USER1, "uprt %p = <id = %u, lcore = <", uprt, uprt->id);
	for (i = 0; i < uprt->nb_lcore; i++)
		if (i < uprt->nb_lcore-1)
			printf("%u,", uprt->lcore[i]);
		else
			printf("%u", uprt->lcore[i]);
	printf(">, mtu = %u, rx_offload = %u, tx_offload = %u,\n"
		"ipv4 = %#x, "
	...
	RTE_LOG(NOTICE, USER1, "Hash Key: ");
	for (i = 0; i < uprt->hash_key_size; i++)
		printf("%02x", uprt->hash_key[i]);
	printf("\n");
}
	...
	uint32_t lcore, uint16_t op, uint32_t bidx)
{
	...
	if (fes->s == NULL) {
		...
		RTE_LOG(ERR, USER1, "stream open failed for port %u with error "
			"code=%u, bidx=%u, lc=%u\n",
			ntohs(l4->sin_port), rc, bidx, becfg.cpu[bidx].id);
		return NULL;
	}
 ...
 */
static int
netbe_find4(const struct in_addr *laddr, uint16_t lport,
	const struct in_addr *raddr, uint32_t be_lc)
{
	uint32_t i, j;
	uint32_t idx, qid;
		...
		for (i = 0; i != becfg.cpu_num; i++) {
			...
			for (j = 0; j != bc->prtq_num; j++) {
				if (laddr->s_addr == bc->prtq[j].port.ipv4) {
					...
					align_nb_q = rte_align32pow2(
						bc->prtq[j].port.nb_lcore);
					qid = (lport % align_nb_q) %
						bc->prtq[j].port.nb_lcore;
					if (bc->prtq[j].rxqid == qid)
						return i;
				}
			}
		}
...
main(int argc, char *argv[])
{
	...
	if (rc != 0)
		rte_exit(EXIT_FAILURE,
			"%s: netbe_lcore_init failed with error code: %d\n",
			__func__, rc);
	...
	for (i = 0; i != becfg.prt_num && rc == 0; i++) {
		...
		if (rc != 0)
			rte_exit(EXIT_FAILURE,
				"%s: update_rss_reta failed with error code: %d\n",
				__func__, rc);
