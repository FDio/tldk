	...
	const struct sockaddr *sa)
{
	...
	FILL_STREAM_PARAM(uprm, fes->fwdprm.local_addr, fes->fwdprm.remote_addr,
		fes->fwdprm.recv_ev, fes->fwdprm.send_ev);
...
netfe_lcore_fini_udp(void)
{
	...
	while (fe->sidx != 0) {
		i = fe->sidx - 1;
		tle_udp_stream_get_param(fe->fs[i].s, &uprm);
		netfe_stream_dump(fe->fs + i, &uprm.local_addr, &uprm.remote_addr);
		netfe_stream_close(fe, 1);
	}
