...
static void
netfe_stream_dump_tcp(const struct netfe_stream *fes)
{
	...
	tle_tcp_stream_get_param(fes->s, &tprm);
	la = &tprm.local_addr;
	ra = &tprm.remote_addr;
...
netfe_lcore_init_tcp(const struct netfe_lcore_prm *prm)
{
	...
	for (i = 0; i != prm->nb_streams; i++) {
		fes = netfe_stream_open_tcp(fe, &prm->stream[i].sprm, lcore,
...
static void
fill_dst_tcp(struct tle_dest *dst, struct netbe_dev *bed,
...
static inline void
netfe_pkt_addr_tcp(const struct rte_mbuf *m, struct sockaddr_storage *ps,
	uint16_t family)
{
...
pkt_eq_addr_tcp(struct rte_mbuf *pkt[], uint32_t num, uint16_t family,
	struct sockaddr_storage *cur, struct sockaddr_storage *nxt)
{
	uint32_t i;

	for (i = 0; i != num; i++) {
		netfe_pkt_addr_tcp(pkt[i], nxt, family);
...
static struct netfe_stream *
find_fwd_dst_tcp(uint32_t lcore, struct netfe_stream *fes,
...
netfe_fwd_tcp(uint32_t lcore, struct netfe_stream *fes)
{
	...
	netfe_pkt_addr_tcp(pkt[0], pi0, family);

	x = 0;
	for (i = 0; i != n; i = j) {

		j = i + pkt_eq_addr_tcp(&pkt[i + 1],
			n - i - 1, family, pi0, pi1) + 1;

		fed = find_fwd_dst_tcp(lcore, fes, (const struct sockaddr *)pi0);
		if (fed != NULL) {
...
static inline int
check_connection_permission(__rte_unused struct rte_mbuf *mbuf)
{
	/*
	 * TODO: Add logic to check the stream permission
	 */
	return 0;
}
	...
	struct netfe_stream *fes)
{
	...
	n = tle_tcp_stream_synreqs(fes->s, syn_reqs, MAX_PKT_BURST);
	if (n == 0)
		return;
	...
	for (i = 0, k = 0; i != n; i++) {
		...
		rc = check_connection_permission(syn_reqs[i].pkt);
		if (rc == 0) {
			acpt_prm[k].syn = syn_reqs[i];
			acpt_prm[k].prm.recv_ev = tle_event_alloc(fe->rxeq,
				&fe->fs[sidx + k]);
			acpt_prm[k].prm.send_ev = tle_event_alloc(fe->txeq,
				&fe->fs[sidx + k]);
	...
	tle_tcp_reject(fes->s, rej_reqs, n - k);

	/* accept k new connections */
	rc = tle_tcp_stream_accept(fes->s, acpt_prm, s, k);
	if (rc != 0) {
...
static inline void
netfe_rx_process_tcp(__rte_unused uint32_t lcore, struct netfe_stream *fes)
{
	uint32_t k, n;
...
netfe_lcore_tcp(void)
{
	...
	if (n != 0) {
		...
		for (j = 0; j != n; j++)
			...
			netfe_rx_process_tcp(lcore, fs[j]);
	}
...
static inline void
netbe_rx_tcp(struct netbe_lcore *lc, uint32_t pidx)
{
	uint32_t j, k, n;
	...
	k = tle_tcp_rx_bulk(lc->prtq[pidx].dev, pkt, rp, rc, n);
...
static inline void
netbe_tx_tcp(struct netbe_lcore *lc, uint32_t pidx)
{
	uint32_t j, k, n;
	struct rte_mbuf **mb;
...
static int
netbe_add_dest_tcp(struct netbe_lcore *lc, uint32_t dev_idx, uint16_t family,
	const struct netbe_dest *dst, uint32_t dnum)
{
	...
	for (i = 0; i != dnum && rc == 0; i++) {
		fill_dst_tcp(dp + i, lc->prtq + dev_idx, dst + i, l3_type, sid);
		if (family == AF_INET)
...
static int
netbe_dest_init_tcp(const char *fname, struct netbe_cfg *cfg)
{
	int32_t rc;
	uint32_t f, i, p;
	uint32_t k, l, cnt;
