	...
	uint32_t lcore, uint16_t op, uint32_t bidx, uint8_t server_mode)
{
	...
	fes->erev = tle_event_alloc(fe->ereq, fes);
	...
	struct netfe_stream *fes)
{
	...
	for (i = 0; i != k; i++) {
		...
		prm = &acpt_prm[i].cfg;
		prm->recv_ev = tle_event_alloc(fe->rxeq, fs[i]);
		prm->send_ev = tle_event_alloc(fe->txeq, fs[i]);
		prm->err_ev = tle_event_alloc(fe->ereq, fs[i]);
	...
	if (rc != n) {
		...
		/* free k - rc allocated events */
		for (i = rc; i != k; i++) {
			tle_event_free(acpt_prm[i].cfg.recv_ev);
			tle_event_free(acpt_prm[i].cfg.send_ev);
			tle_event_free(acpt_prm[i].cfg.err_ev);
		}
	...
	for (i = 0; i != rc; i++) {
		...
		ts->s = rs[i];
		ts->rxev = acpt_prm[i].cfg.recv_ev;
		ts->txev = acpt_prm[i].cfg.send_ev;
		ts->erev = acpt_prm[i].cfg.err_ev;
		ts->op = fes->op;
		ts->proto = fes->proto;
